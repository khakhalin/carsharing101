...
The coefficient $L$ is called "price elasticity". If $L$ is close to zero, demand is called inelastic, as it does not change much regardless of changes in price; in the world of car-sharing this may for example happen if people need to get out of a certain location, and have no other choices but to rent a car (at least at the present moment). If, on the other hand, the $L$ is large, the demand would change a lot even in response to small changes in price, which is called an elastic demand. This situation may be observed, for example, in a presence strong and effective competition, when people can easily optimize their expenses by picking the best option out of those available.
...
This means that the revenue $R$ increases with price $P$ when $L<1$, and the elasticity is low. When choices are limited, customers would be willing to pay more money for the service, and so increasing the price would also increase the profits. Conversely, if $L>1$, the revenue would decrease with price, as the sales would drop as the price increases. In this case it would be more profitable to drop the price somewhat, to boost the sales. If $L=1$, the increase in price is perfectly compensated by a decrease in sales, and the revenue stays the same regardless of price adjustments.
...
At the same time, congestion areas obviously present a challenge for business development and public relationships departments within a company, as it may not be obvious to city officials why they would give a car-sharing service an exception from the congestion fee. But maybe that's where this document, and the logic described here, may become useful. A car-sharing company needs its cars to be returned to central hubs to operate. A financial incentive makes it easier for customers to return these cars. Together, it improves the car-sharing coverage and service, and reduces the number of privately owned cars in the city (REF: Amsterdam?). Hopefully if repeated often enough, these theses would improve the collaboration between mobility providers and modern cities.
