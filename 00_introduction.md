...
If however you preer to dive deeper into the code and math that stands behind each the graphs, you can always track them back to the code! All graphs in this repository are directly generated by scripts stored in the `scripts` folder. To find a script that generated a certain figure you need to search by this figure's name. Say, you got curious about Figure 2.4.2 from section 2.4 in chapter 2. You can right-click on this image, and learn that the png file for this figure is named `02relos_02stations_04fleet_size_few_stations` or something like that (note how the name of the file is similar to figure's number). You can then search across all Jupyter notebooks in the `scripts` folder for this name, as there is a `plt.savefig()` command somewhere that saves this figure on disk. In all (or at least most) simulations the random generator seed is set at the beginning of the script, so re-running the script will genuinely re-generate the image that is linked in the text of the article. Use this feature at your pleasure, but also be remember that if you change the code, the figure will also change! So my recommendation would be to either comment out the `plt.savefig()` command before changing the code, or to hard reset your repo at the end of the coding session 😅
