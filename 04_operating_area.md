...
If a city is profitable (left column), there is wide overlap between the curves, and thus the wide range of OA size values in which the city is profitable. If the city is deeply unprofitable (either your cars are too expensive, or the city consists moslty of low-density surburbs with high car ownership share, which is sadly the case for most North Americna cities), it may be impossible to make it profitable (at least not with a "blob"-shaped home area; it may still be profitable if you cover it with a few "service points", but it is not a scenario we are considering here). As you move from a "non-profitable" to a "profitable" scenario, at some point the curve "touch" (middle column), giving you a situaion when with a rather specific size of a home area you can run the city in a "breaking even" mode.
