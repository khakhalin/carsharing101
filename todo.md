...
* Rework config    
    * Move default config values out and their processor (config intializer) out, into a diff py file.
    * Decide between these two alternatives:
        * Either make it a mixin base class for the city (a city is not a config, but it has config functionality)
        * Or make config into a separate pydantic dataclass, assigning params to it, and not to the simulation. In which case parameters will be referred not as `self.n_steps`, but as `self.cfg.n_steps` (for example)
